package com.ankamagames.atouin.renderers
{
   ...
   public class MapRenderer extends EventDispatcher
   {
      ...
      public function render(param1:DataMapContainer, param2:Boolean = false, param3:uint = 0, param4:Boolean = true) : void
      {
         ...
         for each(_loc16_ in _loc11_)
         {
            if(_loc16_ is NormalGraphicalElementData)
            {
               ...
               else
               {
                  ...
                  {
                     _loc12_ = new Uri(this._gfxPathSwf + "/" + _loc14_.gfxId + ".swf");
                  }
                  _loc9_.push(_loc12_);
         ...
         if(!this._hasGroundJPG && param4)
         {
            for each(_loc17_ in this._map.backgroundFixtures)
            {
               ...
               else
               {
                  ...
                  {
                     _loc12_ = new Uri(this._gfxPathSwf + "/" + _loc17_.fixtureId + ".swf");
                  }
         ...
         {
            this._gfxLoader.load(_loc9_,null,AdvancedSwfAdapter);
         }
      ...
      private function makeMap() : void
      {
         ...
         groundLayerCtr = new Bitmap(new BitmapData(StageShareManager.startWidth * _groundGlobalScaleRatio * this._renderScale,StageShareManager.startHeight * _groundGlobalScaleRatio * this._renderScale,!this._renderBackgroundColor,this._renderBackgroundColor?this._map.backgroundColor:0),"auto",true);
         ...
         for each(layer in this._map.layers)
         {
            ...
            if(layer.cellsCount != 0)
            {
               ...
               {
                  reelBmpDt = new BitmapData(AtouinConstants.RESOLUTION_HIGH_QUALITY.x * this._renderScale,AtouinConstants.RESOLUTION_HIGH_QUALITY.y * this._renderScale,!this._renderBackgroundColor,this._renderBackgroundColor?this._map.backgroundColor:0);
                  m = new Matrix();
      ...
      private function renderFixture(param1:Array, param2:Bitmap) : void
      {
         ...
         for each(_loc5_ in param1)
         {
            ...
            else
            {
               ...
               this._m.translate((_loc5_.offset.x + AtouinConstants.CELL_HALF_WIDTH + _bitmapOffsetPoint.x) * this.renderScale + _loc8_,(_loc5_.offset.y + AtouinConstants.CELL_HEIGHT) * this.renderScale + _loc9_);
      ...
      private function rasterizeSwf(param1:DisplayObject, param2:int = 1) : BitmapData
      {
         ...
         _loc3_.identity();
         var _loc6_:Rectangle = _loc5_.getColorBoundsRect(4.27819008E9,0,false);
