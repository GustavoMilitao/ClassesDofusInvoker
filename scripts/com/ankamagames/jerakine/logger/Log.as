package com.ankamagames.jerakine.logger
{
   ...
   public final class Log extends Object
   {
      ...
      private static function flushBuffer() : void
      {
         ...
         var _loc3_:LogEvent = null;
         var _loc1_:Array = _tempTarget.getBuffer();
         removeTarget(_tempTarget);
         for each(_loc2_ in _targets)
         {
            if(_loc2_ is TemporaryBufferTarget)
            {
               TemporaryBufferTarget(_loc2_).clearBuffer();
               break;
            }
         }
         for each(_loc3_ in _loc1_)
         {
            _dispatcher.dispatchEvent(_loc3_.clone());
         }
