package com.ankamagames.berilia.uiRender
{
   ...
   public class XmlParsor extends EventDispatcher
   {
      ...
      protected function parseMainNode(param1:XMLNode) : UiDefinition
      {
         ...
         var _loc12_:String = _loc4_[XmlAttributesEnum.ATTRIBUTE_LABEL_DEBUG];
         if(_loc5_)
         {
         ...
         if(_loc12_)
         {
            _loc2_.labelDebug = _loc12_ == "true";
         }
         var _loc14_:int = _loc3_.length;
         _loc15_ = 0;
         while(_loc15_ < _loc14_)
         {
            _loc13_ = _loc3_[_loc15_];
            switch(_loc13_.nodeName)
            {
               case XmlTagsEnum.TAG_CONSTANTS:
                  this.parseConstants(_loc13_,_loc2_.constants);
                  break;
               ...
               case XmlTagsEnum.TAG_BUTTON:
                  _loc2_.graphicTree.push(this.parseGraphicElement(_loc13_));
                  break;
               case XmlTagsEnum.TAG_SHORTCUTS:
                  _loc2_.shortcutsEvents = this.parseShortcutsEvent(_loc13_);
                  break;
               default:
                  this._log.warn("[" + this._sUrl + "] " + _loc13_.nodeName + " is not allowed or unknown. " + this.suggest(_loc13_.nodeName,[XmlTagsEnum.TAG_CONTAINER,XmlTagsEnum.TAG_STATECONTAINER,XmlTagsEnum.TAG_BUTTON,XmlTagsEnum.TAG_SHORTCUTS]));
            }
            _loc15_ = _loc15_ + 1;
         }
