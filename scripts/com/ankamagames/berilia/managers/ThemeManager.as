            ...
            for each(_loc3_ in this._themesRoot.getDirectoryListing())
            {
               if(!(!_loc3_.isDirectory || _loc3_.name.charAt(0) == "."))
               {
                  ...
                  if(_loc4_)
                  {
                     ...
                     {
                        _log.debug("Using FileStream to load " + _loc4_.nativePath + " on MAC OS X!");
                        _loc7_ = new FileStream();
                        _loc7_.open(_loc4_,FileMode.READ);
                        _loc8_ = XML(_loc7_.readUTFBytes(_loc7_.bytesAvailable));
                        _loc7_.close();
                        _loc9_ = _loc2_.path.split("/");
                        this.loadDT(_loc8_,_loc2_.fileName.split(".")[0],_loc9_[_loc9_.length - 2]);
                     }
                     else
                     {
      ...
      public function applyTheme(param1:String) : void
      {
         ...
         if(this._dtFileToLoad == this._themeCount)
         {
            ...
            else
            {
               ...
               {
                  var param1:String = this._themeNames[0];
