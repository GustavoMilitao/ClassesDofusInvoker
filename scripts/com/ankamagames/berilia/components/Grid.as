package com.ankamagames.berilia.components
{
   ...
   public class Grid extends GraphicContainer implements FinalizableUIComponent
   {
      ...
      public function set rendererName(param1:String) : void
      {
         if(param1.indexOf(".") == -1)
         {
            var param1:String = "com.ankamagames.berilia.components.gridRenderer." + param1;
         }
      ...
      private function itemExists(param1:*) : Boolean
      {
         ...
         if(this._dataProvider)
         {
            ...
            while(_loc2_ < _loc3_)
            {
               _loc4_ = SecureCenter.unsecure(this._dataProvider[_loc2_]);
               if(_loc4_ === SecureCenter.unsecure(param1))
               {
                  return true;
               }
      ...
      protected function dispatchMessage(param1:Message, param2:MessageHandler = null) : void
      {
         if(!this.silent)
         {
            if(!param2)
            {
               var param2:MessageHandler = Berilia.getInstance().handler;
            }
