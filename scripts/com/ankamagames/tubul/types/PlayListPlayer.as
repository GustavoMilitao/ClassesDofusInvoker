package com.ankamagames.tubul.types
{
   ...
   public class PlayListPlayer extends EventDispatcher
   {
      ...
      public function nextSound(param1:VolumeFadeEffect = null, param2:Boolean = false, param3:Boolean = true) : void
      {
         ...
         {
            _loc4_ = this._sounds.indexOf(this._playingSound);
            if(_loc4_ == this._sounds.length - 1)
            {
      ...
      public function playSilence(param1:VolumeFadeEffect = null, param2:SoundSilence = null) : void
      {
         ...
         if(!param2)
         {
            ...
            var param2:SoundSilence = this._silence;
         }
      ...
      private function playSound(param1:ISound, param2:Boolean = true) : void
      {
         ...
         var _loc6_:VolumeFadeEffect = null;
         var _loc7_:PlaylistEvent = null;
         if(!this._isPlaying)
         {
            this._isPlaying = true;
         }
         ...
         var _loc3_:IAudioBus = this._playingSound.bus;
         if(_loc3_ != null)
         {
            _loc4_ = false;
            ...
            {
               _loc5_ = this._fadeIn.clone();
            }
            else
            {
               _loc5_ = new VolumeFadeEffect(0,1,0);
            }
            if(this._fadeOut && param2)
            {
               _loc6_ = this._fadeOut.clone();
            }
            else
            {
               _loc6_ = new VolumeFadeEffect(1,0,0);
            }
            this._playingSound.play(_loc4_,this._playingSound.totalLoops,_loc5_,_loc6_);
            _loc7_ = new PlaylistEvent(PlaylistEvent.NEW_SOUND);
            _loc7_.newSound = this._playingSound;
