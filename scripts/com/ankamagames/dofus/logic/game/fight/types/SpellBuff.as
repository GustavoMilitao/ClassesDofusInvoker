package com.ankamagames.dofus.logic.game.fight.types
{
   ...
   public class SpellBuff extends BasicBuff
   {
      ...
      override public function onApplyed() : void
      {
         ...
         var _loc7_:CharacterBaseCharacteristic = null;
         ...
         if(_loc1_)
         {
            ...
            for each(_loc3_ in _loc1_.spellModifications)
            {
               if(this.spellId == _loc3_.spellId)
               {
                  if(_loc3_.modificationType == this.modifType)
                  {
                     ...
                     if(stack)
                     {
                        for each(_loc6_ in stack)
                        {
                           if(_loc6_ is SpellBuff)
                           {
                              _loc3_.value.contextModif = _loc3_.value.contextModif + (_loc6_ as SpellBuff).delta;
                           }
                        }
                     }
                     else
            ...
            if(!_loc2_)
            {
               _loc7_ = new CharacterBaseCharacteristic();
               _loc7_.base = 0;
               _loc7_.additionnal = 0;
               _loc7_.alignGiftBonus = 0;
               _loc7_.contextModif = this.delta;
               _loc7_.objectsAndMountBonus = 0;
               _loc8_ = new CharacterSpellModification();
               _loc8_.modificationType = this.modifType;
               _loc8_.spellId = this.spellId;
               _loc8_.value = _loc7_;
               _loc1_.spellModifications.push(_loc8_);
            }
