      ...
      public function refreshTarget(param1:Boolean = false) : void
      {
         ...
         if(_currentTargetIsTargetable)
         {
            ...
            if(this._invocationPreview.length > 0)
            {
               ...
               {
                  _loc14_ = MapPoint.fromCellId(_loc8_.disposition.cellId).x;
                  _loc15_ = MapPoint.fromCellId(_loc8_.disposition.cellId).y;
                  _loc16_ = MapPoint.fromCellId(_loc8_.disposition.cellId).distanceTo(MapPoint.fromCellId(this._currentCell));
                  _loc17_ = [MapPoint.fromCoords(_loc14_ + _loc16_,_loc15_),MapPoint.fromCoords(_loc14_ - _loc16_,_loc15_),MapPoint.fromCoords(_loc14_,_loc15_ + _loc16_),MapPoint.fromCoords(_loc14_,_loc15_ - _loc16_)];
                  _loc18_ = 0;
                  while(_loc18_ < 4)
                  {
                     _loc21_ = this._invocationPreview[_loc18_];
                     _loc19_ = _loc21_ as TiphonSprite;
                     if(_loc19_ && _loc19_.getSubEntitySlot(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_MOUNT_DRIVER,0) && !_loc19_.getSubEntityBehavior(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_MOUNT_DRIVER))
                     {
                        _loc19_.setSubEntityBehaviour(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_MOUNT_DRIVER,new RiderBehavior());
                     }
                     (_loc21_ as AnimatedCharacter).visible = true;
                     _loc21_.position = _loc17_[_loc18_];
                     (_loc21_ as AnimatedCharacter).setDirection(MapPoint.fromCellId(_loc8_.disposition.cellId).advancedOrientationTo(_loc21_.position,true));
                     if(this.isValidCell(_loc21_.position.cellId) && _loc21_.position.cellId != this._currentCell)
                     {
                     ...
                     {
                        (_loc21_ as AnimatedCharacter).visible = false;
                     }
                     _loc18_++;
                  }
               }
               ...
               {
                  _loc20_ = this._invocationPreview[0];
                  (_loc20_ as AnimatedCharacter).visible = true;
                  _loc20_.position = MapPoint.fromCellId(this._currentCell);
                  (_loc20_ as AnimatedCharacter).setDirection(MapPoint.fromCellId(_loc8_.disposition.cellId).advancedOrientationTo(MapPoint.fromCellId(this._currentCell),true));
                  _loc19_ = _loc20_ as TiphonSprite;
                  if(_loc19_ && _loc19_.getSubEntitySlot(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_MOUNT_DRIVER,0) && !_loc19_.getSubEntityBehavior(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_MOUNT_DRIVER))
                  {
                     _loc19_.setSubEntityBehaviour(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_MOUNT_DRIVER,new RiderBehavior());
                  }
                  (_loc20_ as AnimatedCharacter).display(PlacementStrataEnums.STRATA_PLAYER);
               }
            }
            this.showTargetsTooltips(this._targetSelection);
            this.showReplacementInvocationPreview();
         ...
         else
         {
            if(this._invocationPreview.length > 0)
            {
               for each(_loc21_ in this._invocationPreview)
               {
                  (_loc21_ as AnimatedCharacter).visible = false;
               }
            }
         ...
         if(_loc1_ && (!_loc1_.spellLevelInfos.needTakenCell || this._currentCellEntity))
         {
            ...
            for each(_loc3_ in _loc2_)
            {
               if(TELEPORTATION_EFFECTS.indexOf(_loc3_.effectId) != -1)
               {
                  ...
                  _loc5_ = SpellZoneManager.getInstance().getZone(_loc3_.zoneShape,_loc3_.zoneSize as uint,_loc3_.zoneMinSize as uint,false,_loc3_.zoneStopAtTarget as uint);
            ...
            if(_loc11_.length > 0)
            {
               ...
               this._fightTeleportationPreview = new FightTeleportationPreview(this._usedWrapper,_loc11_,_loc14_,_loc7_,_loc16_,_loc15_ > 1,_loc11_.length == _loc8_.length);
