package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class ChatFrame extends Object implements Frame
   {
      ...
      public function pushed() : Boolean
      {
         ...
         var _loc8_:OptionManager = null;
         var _loc9_:* = 0;
         var _loc11_:* = 0;
         var _loc12_:SmileyWrapper = null;
         var _loc13_:Date = null;
         var _loc14_:Array = null;
         var _loc15_:MoodSmileyRequestAction = null;
         ...
         for each(_loc2_ in this._aSmileyPacks)
         {
            for each(_loc11_ in _loc2_.smileys)
            {
               _loc1_ = Smiley.getSmileyById(_loc11_);
               if(_loc1_)
               {
                  _loc12_ = SmileyWrapper.create(_loc1_.id,_loc1_.gfxId,_loc2_.id,_loc1_.categoryId);
                  this._aSmilies.push(_loc12_);
               }
            }
         }
         for(_loc3_ in this._aChannels)
         {
            this._aMessagesByChannel[this._aChannels[_loc3_].id] = new Array();
            this._aParagraphesByChannel[this._aChannels[_loc3_].id] = new Array();
         }
         ...
         _loc4_ = XmlConfig.getInstance().getEntry("config.lang.current");
         _loc5_ = CensoredWord.getCensoredWords();
         _loc6_ = 0;
         for each(_loc7_ in _loc5_)
         {
            if(_loc7_)
            {
               if(_loc7_.language == _loc4_)
               {
                  _loc6_++;
                  if(_loc7_.deepLooking)
                  {
                     this._aCensoredWords[_loc7_.word] = 2;
                  }
                  else
                  {
                     this._aCensoredWords[_loc7_.word] = 1;
                  }
               }
            }
         }
         _loc8_ = OptionManager.getOptionManager("chat");
         _loc9_ = PlayedCharacterManager.getInstance().id;
         if(!_loc8_["favoriteSmileys"])
         {
            _loc8_.add("favoriteSmileys",[]);
         }
         if(!_loc8_["moodSmiley_" + _loc9_])
         {
            _loc8_.add("moodSmiley_" + _loc9_,"");
         }
         var _loc10_:String = _loc8_["moodSmiley_" + _loc9_];
         if(_loc10_ && _loc10_ != "")
         {
            _loc13_ = new Date();
            _loc14_ = _loc10_.split("_");
            if(int(_loc14_[0]) > 0 && Number(_loc14_[1]) < _loc13_.time + 604800000)
            {
               _loc15_ = new MoodSmileyRequestAction();
               _loc15_.smileyId = int(_loc14_[0]);
               this.process(_loc15_);
            }
         }
         return true;
      }
         ...
         switch(true)
         {
            ...
            case msg is ChatSmileyMessage:
               ...
               if(scmsg.entityId == PlayedCharacterManager.getInstance().id)
               {
                  ...
                  if(!chatOptS["favoriteSmileys"])
                  {
                     chatOptS.add("favoriteSmileys",[scmsg.smileyId]);
                  }
                  else
                  {
                     ...
                     chatOptS["favoriteSmileys"] = favSmileys;
                  }
               }
            ...
            case msg is ExchangeGuildTaxCollectorGetMessage:
               ...
               taxcollectorCollectedMsg = "{taxcollectorCollected," + taxCollectorWrapper.uniqueId + "::" + PatternDecoder.combine(I18n.getUiText("ui.social.taxCollector.collected",[egtcgmsg.userName,totalQuantity]),"n",totalQuantity <= 1) + "}";
               KernelEventsManager.getInstance().processCallback(ChatHookList.TextInformation,taxcollectorCollectedMsg,ChatActivableChannelsEnum.CHANNEL_GUILD,this.getTimestamp(),false);
               return true;
      ...
      private function displayCarac(param1:CharacterBaseCharacteristic) : String
      {
         ...
         return param1.base + param1.additionnal + " (" + _loc3_ + _loc2_ + ")";
      }
      ...
      {
         var _loc3_:* = 0;
         var _loc4_:Boolean = Console.getInstance().opened && Console.getInstance().chatMode;
         _loc3_ = 0;
         while(_loc3_ < param1)
         {
            if(!this._aHistoryMessagesByChannel[param2])
            {
               this._aHistoryMessagesByChannel[param2] = new Array();
            }
            if(_loc4_ && _loc3_ < this._maxMessagesStored)
            {
               this._aHistoryMessagesByChannel[param2].push(this._aMessagesByChannel[param2].shift());
            }
            else
            {
