package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class StackManagementFrame extends Object implements Frame
   {
      ...
      public function onKeyDown(param1:KeyboardEvent) : void
      {
         ...
         if(this._enabled && !this._keyDown && param1.keyCode == KEY_CODE)
         {
            this._keyDown = true;
      ...
      public function process(param1:Message) : Boolean
      {
         ...
         var _loc3_:AddBehaviorToStackAction = null;
         var _loc4_:RemoveBehaviorToStackAction = null;
         var _loc5_:AbstractBehavior = null;
         ...
         var _loc11_:AbstractBehavior = null;
         var _loc12_:MoveBehavior = null;
         var _loc13_:ChangeMapBehavior = null;
         var _loc14_:InteractiveElementBehavior = null;
         var _loc15_:* = 0;
         var _loc16_:InteractiveElementActivationMessage = null;
         var _loc17_:* = false;
         if(param1 is GameContextCreateMessage)
         {
            _loc2_ = param1 as GameContextCreateMessage;
            if(_loc2_.context == GameContextEnum.FIGHT)
            {
            ...
            else if(_loc2_.context == GameContextEnum.ROLE_PLAY)
            {
               this._enabled = true;
            }
         }
         ...
         switch(true)
         {
            case param1 is AddBehaviorToStackAction:
               _loc3_ = param1 as AddBehaviorToStackAction;
               for each(var _loc20_ in _loc3_.behavior)
               {
                  switch(_loc20_)
                  {
            ...
            case param1 is RemoveBehaviorToStackAction:
               _loc4_ = param1 as RemoveBehaviorToStackAction;
               switch(_loc4_.behavior)
               {
            ...
            case param1 is EmptyStackAction:
               ...
               return true;
            default:
               _loc6_ = getQualifiedClassName(param1).split("::")[1];
               _loc7_ = _loc6_ == "InteractiveUsedMessage" || _loc6_ == "InteractiveUseErrorMessage";
               if(this._paused && (ACTION_MESSAGES.indexOf(_loc6_) != -1 || _loc7_))
               {
                  for each(_loc11_ in this._stackOutputMessage)
                  {
                     switch(true)
                     {
                        case _loc11_ is MoveBehavior:
                           _loc12_ = _loc11_ as MoveBehavior;
                           if(param1 is CellClickMessage && (param1 as CellClickMessage).cellId == _loc12_.getMapPoint().cellId)
                           {
                              this._waitingMessage = param1;
                              return false;
                           }
                           continue;
                        case _loc11_ is ChangeMapBehavior:
                           _loc13_ = _loc11_ as ChangeMapBehavior;
                           if(param1 is AdjacentMapClickMessage && (param1 as AdjacentMapClickMessage).cellId == _loc13_.getMapPoint().cellId)
                           {
                              this._waitingMessage = param1;
                              return false;
                           }
                           continue;
                        case _loc11_ is InteractiveElementBehavior:
                           _loc14_ = _loc11_ as InteractiveElementBehavior;
                           if(param1 is InteractiveElementActivationMessage && (param1 as InteractiveElementActivationMessage).interactiveElement.elementId == _loc14_.interactiveElement.elementId)
                           {
                              this._waitingMessage = param1;
                              return false;
                           }
                           if(_loc7_ && _loc14_.interactiveElement.elementId == (param1 as Object).elemId)
                           {
                              _loc14_.processOutputMessage(param1,this._currentMode);
                              this.removeCheckPoint(_loc14_);
                              _loc15_ = this._stackOutputMessage.indexOf(_loc14_);
                              _loc16_ = this._waitingMessage as InteractiveElementActivationMessage;
                              if(_loc16_ && _loc16_.interactiveElement.elementId == _loc14_.interactiveElement.elementId)
                              {
                                 this._waitingMessage = null;
                              }
                              if(_loc15_ != -1)
                              {
                                 this._stackOutputMessage.splice(_loc15_,1);
                                 return false;
                              }
                           }
                           continue;
               ...
               {
                  _loc5_.checkAvailability(param1);
               }
               if(this._stopMessages.indexOf(_loc6_) != -1)
               {
                  _loc17_ = true;
               ...
               _loc8_ = this.processStackInputMessages(param1);
               _loc9_ = this.processStackOutputMessages(param1);
               return _loc8_;
         }
      }
      ...
      private function processStackOutputMessages(param1:Message) : Boolean
      {
         ...
         if(this._stackOutputMessage.length > 0)
         {
            ...
            else if(_loc2_.pendingMessage != null)
            {
      ...
      public function removeAction(param1:AbstractBehavior) : void
      {
         this.removeCheckPoint(param1);
         param1.remove();
         this._stackOutputMessage.splice(this._stackOutputMessage.indexOf(param1),1);
      }
