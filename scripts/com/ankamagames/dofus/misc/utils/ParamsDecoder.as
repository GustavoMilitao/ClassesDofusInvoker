package com.ankamagames.dofus.misc.utils
{
   ...
   public class ParamsDecoder extends Object
   {
      ...
      private static function processReplace(param1:String, param2:String, param3:Array) : String
      {
         ...
         var _loc27_:Date = null;
         var _loc28_:uint = 0;
         var _loc29_:Companion = null;
         ...
         else
         {
            switch(param1)
            {
               case "$item":
                  ...
                  if(_loc6_)
                  {
                     _loc30_ = ItemWrapper.create(0,0,param3[_loc5_],0,null,false);
                     _loc4_ = HyperlinkItemManager.newChatItem(_loc30_);
                  }
                  else
                  {
               ...
               case "$time":
                  _loc27_ = new Date();
                  _loc28_ = param3[_loc5_] * 1000 - _loc27_.time;
                  _loc4_ = TimeManager.getInstance().getDuration(_loc28_);
                  break;
               case "$companion":
               case "$sidekick":
                  _loc29_ = Companion.getCompanionById(param3[_loc5_]);
                  if(_loc29_)
                  {
                     _loc4_ = _loc29_.name;
                  }
