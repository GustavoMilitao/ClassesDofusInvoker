   ...
   public class SpellLevel extends Object implements ICellZoneProvider, IDataCenter
   {
      ...
      public function get spellZoneEffects() : Vector.<IZoneShape>
      {
         ...
         if(!this._spellZoneEffects)
         {
            ...
            {
               _loc2_ = new ZoneEffect(uint(_loc1_.zoneSize),_loc1_.zoneShape);
      ...
      public function get canSummon() : Boolean
      {
         ...
         for each(_loc1_ in this.effects)
         {
            if(_loc1_.effectId == ActionIdConverter.ACTION_CHARACTER_ADD_DOUBLE || _loc1_.effectId == ActionIdConverter.ACTION_SUMMON_SLAVE)
            {
               return true;
            }
            if(_loc1_.effectId == ActionIdConverter.ACTION_SUMMON_CREATURE || _loc1_.effectId == ActionIdConverter.ACTION_KILL_AND_SUMMON_CREATURE || _loc1_.effectId == ActionIdConverter.ACTION_KILL_AND_SUMMON_SLAVE)
            {
      ...
      public function get canBomb() : Boolean
      {
         ...
         for each(_loc1_ in this.effects)
         {
            if(_loc1_.effectId == ActionIdConverter.ACTION_SUMMON_BOMB)
            {
               return true;
            }
            if(_loc1_.effectId == ActionIdConverter.ACTION_SUMMON_CREATURE || _loc1_.effectId == ActionIdConverter.ACTION_KILL_AND_SUMMON_CREATURE || _loc1_.effectId == ActionIdConverter.ACTION_KILL_AND_SUMMON_SLAVE)
      ...
      public function get canThrowPlayer() : Boolean
      {
         ...
         for each(_loc1_ in this.effects)
         {
            if(_loc1_.effectId == ActionIdConverter.ACTION_THROW_CARRIED_CHARACTER)
            {
               return true;
            }
         }
