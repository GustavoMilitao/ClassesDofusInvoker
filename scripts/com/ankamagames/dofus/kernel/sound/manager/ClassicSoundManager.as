package com.ankamagames.dofus.kernel.sound.manager
{
   ...
   public class ClassicSoundManager extends EventDispatcher implements ISoundManager
   {
      ...
      public function setSubArea(param1:Map = null) : void
      {
         ...
         var _loc9_:IAudioBus = null;
         var _loc10_:IAudioBus = null;
         ...
         if(this.soundIsActivate && false)
         {
            if(this._indoor == MapTypesEnum.INDOOR && param1.mapType == MapTypesEnum.OUTDOOR)
            {
               ...
               _loc7_.removeEffect(this._lowPassFilter);
               _loc8_.removeEffect(this._lowPassFilter);
            }
            else if(this._indoor == MapTypesEnum.OUTDOOR && param1.mapType == MapTypesEnum.INDOOR)
            {
               this._indoor = MapTypesEnum.INDOOR;
               _loc9_ = Tubul.getInstance().getBus(TubulSoundConfiguration.BUS_MUSIC_ID);
               _loc10_ = Tubul.getInstance().getBus(TubulSoundConfiguration.BUS_AMBIENT_2D_ID);
               _loc9_.addEffect(this._lowPassFilter);
               _loc10_.addEffect(this._lowPassFilter);
            }
         }
         ...
         for each(_loc6_ in _loc3_.playlists)
         {
            if(_loc6_[0] == PlaylistTypeEnum.ROLEPLAY_MUSIC)
            {
               _loc4_ = Playlist.getPlaylistById(_loc6_[1]);
            }
            else if(_loc6_[0] == PlaylistTypeEnum.AMBIANCE_MUSIC)
            {
               _loc5_ = Playlist.getPlaylistById(_loc6_[1]);
            }
         }
         this._ambientManager.setAmbientSounds(_loc3_.ambientSounds,_loc2_.sounds,_loc4_,_loc5_);
      ...
      public function playStopableSound(param1:String) : void
      {
         ...
         if(this._forceSounds)
         {
            ...
            _loc5_ = SoundFactory.getSound(EnumSoundType.UNLOCALIZED_SOUND,_loc4_);
            _loc5_.busId = _loc2_;
